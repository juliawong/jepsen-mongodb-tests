...
;; The start-daemon! function is adapted from jepsen.control.util/start-daemon!
;; with some additional changes to work better with wrapper scripts (a la
;; jepsen.mongodb.time/wrap!). The issues mentioned below are what motivated
;; this adaptation.
;;
;;   - jepsen.control.util/start-daemon! doesn't support disabling the creation
;;     of the pidfile. The :make-pidfile? option is exposed to allow the caller
;;     to specify the desired behavior.
;;
;;   - jepsen.control.util/start-daemon! forces the executable `bin` to match
;;     the process represented by the pidfile. The :match-executable? option is
;;     exposed to allow the called to specify the desired behavior.
;;
;; These issues prevented the --pidfilepath option to mongod from being used to
;; create the pidfile and led to start-stop-daemon always attempting to launch a
;; mongod process.
(defn start-daemon!
  ...
  :process-name"
  [opts bin & args]
  (info "starting" (.getName (io/file bin)))
  (apply c/exec :start-stop-daemon :--start
...
;; DB Hash checking operations

(defn dbhash-shell-cmd [node]
  "Constructs a mongo shell command string for running a db hash check on a
   replica set. We make sure the replica set has stabilized with node
   0 as primary, since our Jepsen tests give node 0 the highest priority."
  (let [cmd-lines
          [(format "var rst = new ReplSetTest('%s')" (name node)),
           "rst.waitForState(rst.nodes[0], ReplSetTest.State.PRIMARY)"
           "rst.checkReplicatedDataHashes();"]]
         (str/join "; " cmd-lines)))

(defn invoke-dbhash-validate!
  "Execute db hash validation on a replica set by using a mongo shell on a host
   node to execute the built in 'checkReplicatedDataHashes' function. We
   determine success or failure of the check simply by observing the exit code
   of the shell process. Returns the updated op indicating success or failure."
  [test op]
  (let [node (first (:nodes test)) ; use the first node
        shell-cmd (dbhash-shell-cmd node)
        session (get (:sessions test) node)
        mongo "/opt/mongodb/bin/mongo"]
        (try
          ; Run the db hash check through the mongo shell.
          ; It will throw if the command returns a non-zero exit code.
          (do (c/with-session node session
                (c/exec mongo :--nodb :--quiet :--eval shell-cmd))
                (assoc op :result :valid))
        (catch RuntimeException e
          ; Capture the output on failure
          (assoc op
              :result :invalid
              :fail-msg (.getMessage e))))))

(def dbhash-gen
  "db hash check op generator"
  (gen/once {:type :info, :f :dbhash-check, :value nil}))

(def dbhash-client
  "A client that runs a replica set db hash check."
  (reify client/Client
    (setup! [this _ _] this)
    (invoke! [this test op]
        (case (:f op)
          :dbhash-check (invoke-dbhash-validate! test op)))
    (teardown! [this test] this)))

(def dbhash-checker
  "Checker for validating replica set db hashes."
  (reify checker/Checker
    (check [this test model history opts]
      (let [dbhash-result (->> history
                              (filter #(= :dbhash-check (:f %)))
                              ; We only want the result.
                              (filter (fn [op] (contains? op :result)))
                              ; Will be nil if db hash op doesn't exist.
                               first)]
        (if (nil? dbhash-result)
            { :valid? :unknown
              :error  "db hash check was not performed."}
            (case (:result dbhash-result)
              :valid {:valid? true}
              :invalid
                  {:valid? false
                   :error (:fail-msg dbhash-result)}))))))
...
(defn mongodb-test
  ...
  At the end of every test, we execute a db hash validation check. We extend
  the nemesis to handle db hash operations, since it is easy to compose nemesis
  clients. We also extend the generator and checker accordingly."
  [name opts]
  (info "MongoDB expanded test options:\n"
  ...
  (merge
    (assoc tests/noop-test
           ...
           :nemesis         (nemesis/compose
                              ;; We allow for dbhash checks by composing our standard 'chaos'
                              ;; nemesis with a client that handles db hash ops.
                              {#{:isolate :kill :stop} (primary-divergence-nemesis (:clock opts))
                               #{:dbhash-check} dbhash-client})
