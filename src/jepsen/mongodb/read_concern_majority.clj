(ns jepsen.mongodb.read-concern-majority
  ...
  (:require [clojure.tools.logging :refer [info debug warn]]
...
(defrecord Client [db-name coll-name read-concern write-concern client coll]
  ...
  (invoke! [this test op]
    ...
    (with-errors op #{:read}
      (case (:f op)
        :add
          (do (m/insert! coll (select-keys op [:value]))
              (assoc op :type :ok))
...
(defn subset-chain?
  ...
  (let [subset?-pairs (map (partial apply clojure.set/subset?) overlapping-pairs)]
       (every? true? subset?-pairs))))

(def rcmajority-checker
...
(defn test
  "A read concern majority test. We continuously insert documents on many
   ...
   single, dedicated reader thread/process, in order to ensure that our reads
      ...
      {:client (client opts)
       ...
       :checker (checker/compose
                  {:read-concern-majority    rcmajority-checker
                   ...
                   :perf     (checker/perf)})}
      ;; The clients for this test always uses a :majority read concern, so we make sure that
      ;; the options for the MongoDB test are also set to :majority read concern,
      ;; so that the mongod processes are started with --enableMajorityReadConcern.
